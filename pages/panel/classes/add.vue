...
const classData = ref<{ name: string; grade: number | null; group: string; specialization: string }>({
  name: "",
  grade: null,
  group: "",
  specialization: ""
...
const checkForErrors = (): void => {
  ...
  if (classData.value.specialization.length < 1) {
    errors.value.specialization = "Specializace třídy je povinná.";
  }
};
      ...
      switch (resCode) {
        ...
        case "9031":
          if ((response._data.data.taskIds || []).length > 1) {
            alertsStore.addAlert({type: "error", title: "Odstranění tříd", message: `Třídy nebyly odstraněny. Tyto úkoly používají některé z odstraňovaných tříd: ${response._data.data.badIds.join(", ")}`});
            break;
          } else if ((response._data.data.userIds || []).length > 1) {
            alertsStore.addAlert({type: "error", title: "Odstranění tříd", message: `Třídy nebyly odstraněny. Tito uživatelé používají některé z odstraňovaných tříd: ${response._data.data.badIds.join(", ")}`});
            break;
          }

          alertsStore.addAlert({type: "success", title: "Odstranění tříd", message: `Třídy byly úspěšně odstraněny. (0)`});
    ...
    <template #content>
      <div id="classes">
        <div class="content">
          ...
          <div class="form" @input="checkForErrors">
            <div class="section">
              ...
              <div class="content">
                ...
                <input type="text" id="name" placeholder="V1B-ANJ1" v-model="classData.name" />
              ...
              <div class="content">
                ...
                <div class="dropdown">
                  ...
                  <div class="dropdown-content" v-show="open">
                    <div
                        v-for="item in allSpecializations"
                        :key="item.id"
                        class="item"
                        :class="{ selected: selectedSpecialization === item.id }"
                        @click="selectedSpecialization = item.id"
                        v-if="allSpecializations.length > 0"
                    >
                      <Icon class="icon" :name="selectedSpecialization === item.id ? icons.select : icons.selected" />
                      <span><span class="name" v-if="item.name">{{ item.name + " - " }}</span>{{ item.abbreviation }} - Délka studia (roky): {{ item.lengthOfStudy }}</span>
                    </div>
            ...
            <div class="section">
              ...
              <div class="content">
                ...
                <input type="number" id="grade" placeholder="1" min="1" v-model="classData.grade" />
            ...
            <div class="section">
              ...
              <div class="content">
                ...
                <input type="text" id="group" placeholder="A" v-model="classData.group" />
          ...
          <div class="buttons">
            <button type="submit" @click="removeClasses">
              Uložit změny
